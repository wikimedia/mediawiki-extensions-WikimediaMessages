	...
	UploadForm_initialHook
{
	...
	public static function onMobileLicenseLink( &$link, $context, array $attribs, &$msg, &$plural = null ) {
		...
		// Set $link. This affects the page footer and the editor.
		// On Wikidata and Commons, it has no visible effect on the page footer, because the customized
		// messages (above) don't use the parameter.
		switch ( $licensing ) {
			case 'commons':
			case 'standard':
				// Almost all Wikimedia wikis using CC-BY-SA 3.0 are also dual-licensed under GFDL.
			...
			case 'wikidata':
			case 'wikinews':
				// Use the default links generated by MobileFrontend.
				break;
			default:
				throw new ConfigException( "Unknown value for WikimediaMessagesLicensing: '$licensing'" );
		}
